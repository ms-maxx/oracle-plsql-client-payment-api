create or replace package common_pack is
  ...
  */
  ...
  pragma exception_init(e_inactive_object, c_error_code_inactive_object);
  ...
  pragma exception_init(e_object_already_locked,
                        c_error_code_object_already_locked);

  -- Включение/отключение разрешения менять вручную данные объектов "CLIENT"
